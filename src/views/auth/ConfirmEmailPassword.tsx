...
export default function ConfirmEmailPassword() {
    ...
    // 1. استخراج الدوان والمتغيرات من الهوك (إضافة دالة إعادة الإرسال وحالتها إن وجدت)
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // 2. إرسال كود OTP إلى السيرفر عبر الهوك الموجه للمسار /api/Auth/ConfirmEmail
        confirmOtp({ code: otpValue });
    };

    // دالة التعامل مع إعادة إرسال الكود للمسار /api/Auth/ResendOtpCode
    const handleResendOtp = () => {
        if (!userEmail) {
            setLocalError('لم نتمكن من العثور على البريد الإلكتروني لإعادة الإرسال.');
            return;
        }

        // استدعاء الـ Mutation وتمرير الكائن المطلوب للسيرفر
        ...
        <div className="animate-fade-in-up delay-100 text-center lg:text-right">
            ...
            <div className="mt-8 text-center text-sm text-gray-500 flex flex-col items-center gap-2">
                ...
                <button
                    ...
                    className={`font-bold text-moda-purple hover:underline flex items-center gap-1 ${isResending ? 'opacity-50 cursor-not-allowed' : ''
                        }`}
                    type="button"
                >
                    {isResending ? (
